...
def detect_eyeglases(detector, predictor, show=0):

    cap = cv2.VideoCapture(0)
    result = None
    flag = ""
    if flag is None:
        flag = input("Are you wearing Thin Frame or Frameless Spects Y/N:")
    no_glass_frame_count = 200
    frame_count = 120
    while cap.isOpened():
        ...
        for i, rect in enumerate(rects):
            ...
            judge = judge_eyeglass(aligned_face, flag)
            if judge == True:
                cv2.putText(
                    img,
                    "With Glasses",
                ...
                no_glass_frame_count -= 1
        if show == 1:
            cv2.imshow("Result", img)

        if frame_count == 0:
            print("You are wearing spects go ahead and start working")
            result = True
            break
        elif no_glass_frame_count == 0:
            print(
